...
int ll_remove(LinkedList* this,int index)
{
    ...
    Node* pPrevNode=getNode(this,index-1);
    int len=ll_len(this);

    if (this!=NULL && index<len && index >=0 && aux !=NULL)
    {
    	if(index==0)
    	{
    	...
    	else
    	{
    		if (pPrevNode!=NULL)
    		{
    			pPrevNode->pNextNode=aux->pNextNode;
    			free(aux);
    			returnAux=0;
    			this->size--;
    		}
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    int len=ll_len(this);
    int i;
    if(this!=NULL && from >=0 && from < len && to <=len && to >= from)
    {
    ...
    if(this!=NULL && pFunc!=NULL && (order == 0 || order == 1) && len>=0)
    {
    	...
    	for (i=0;i<len-1;i++)
    	{
    		for (j=i+1;j<len;j++)
    		{
    			...
    			retorno=pFunc(auxA,auxB);
    			if (retorno==1 && order ==1)
    			{
    				ll_set(this,i,auxB);
    				ll_set(this,j,auxA);
    			}
